...
def command(self, msg):
    ...
    if not args:
        return reflow("""\
            ...
            \x1Fnick\x1F.
            """.format(keywords='\x02, \x02'.join(sorted(keywords.iterkeys())),
                       help=msg.subaction))
    if args[0] in keywords:
        # FIXME:  Detect whether or not anything was actually returned
        # and display a default "No further help is available" message
        # if not.
        return keywords[args[0]].respond_to(msg._replace(
            action='cmdhelp', subaction=args[0], content=''.join(args[1:])))
    return 'There is no command with the keyword \x02{}\x02.'.format(args[0])

@default.on('cmdhelp')
def cmdhelp(self, msg):
    return reflow("""\
        \x02{}\x02 [\x1Fkeyword\x1F] - List available command keywords,
